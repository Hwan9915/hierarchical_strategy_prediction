"""Stage (identifier) training. GPU 0 only. turn=5 for all modules except comforting."""
import argparse
import os
...
STAGE_MODULES = ["exploration_support_identifier", "comforting_action_identifier"]
...
def train_single(args: argparse.Namespace, module_name: str) -> None:
    ...
    if not args.disable_tb:
        tb_logger = TensorBoardLogger(
            ...
            name=f"stage_{module_name}_{encoder_slug(args.encoder_name)}",
        )
    ...
    if args.verbose:
        print(f"[Stage][Train] turn={turn} {module_name} start")
        if tb_logger is not None:
            print(f"[Stage][TB] log_dir: {tb_logger.log_dir}")
    trainer.fit(model, train_loader, valid_loader)
    trainer.test(ckpt_path="best", dataloaders=test_loader, verbose=bool(args.verbose))

    if args.verbose:
        print(f"[Stage][Train] turn={turn} {module_name} done")
...
def get_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--demo", action="store_true", help="Short demo run (max_epochs=1, limit_*_batches=0.1)")
    return p
...
def main() -> None:
    ...
    if args.verbose:
        print(f"[Stage][Train] encoder: {args.encoder_name} (demo={args.demo})")

    modules_to_run = [args.module] if args.module else STAGE_MODULES
    if args.module and args.module not in STAGE_MODULES:
        raise ValueError(f"--module must be one of {STAGE_MODULES}, got {args.module}")
